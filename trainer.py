...
def parser():
    ...
    args.add_argument('--loadAudioSeconds', type=float, default=3,
                      help='Number of seconds of audio to load for each training sample')
...
def main(args):
    ...
    while (1):
        ...
        if epoch % args.testInterval == 0:
            ...
            scoreFile.write(
                "%d epoch, LR %f, LOSS %f, mAP %2.2f%%, bestmAP %2.2f%%\n" % (epoch, lr, loss, mAPs[-1], max(mAPs)))
            scoreFile.flush()
